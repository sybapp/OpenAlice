...
import { computeNextRun, type CronSchedule } from '../cron/engine.js'
import type {
  TraderFirePayload,
  TraderJob,
  TraderJobCreate,
...
export function createTraderJobEngine(opts: TraderJobEngineOpts): TraderJobEngine {
  const { eventLog } = opts
  const storePath = opts.storePath ?? 'runtime/strategies/jobs.json'
  ...
  async function load(): Promise<void> {
    try {
      ...
      const parsed = JSON.parse(raw) as { jobs?: TraderJob[] }
  ...
  async function fireJob(job: TraderJob, currentMs: number): Promise<void> {
  ...
  return {
    ...
    async add(params: TraderJobCreate) {
      ...
      jobs.push({
        ...
        createdAt: currentMs,
        state: {
          nextRunAtMs: computeNextRun(schedule, currentMs),
          lastRunAtMs: null,
          lastStatus: null,
          consecutiveErrors: 0,
        },
    ...
    async update(id, patch: TraderJobPatch) {
      const job = jobs.find((entry) => entry.id === id)
      if (!job) throw new Error(`trader job not found: ${id}`)
      if (patch.name !== undefined) job.name = patch.name
      if (patch.enabled !== undefined) job.enabled = patch.enabled
      if (patch.strategyId !== undefined) job.strategyId = patch.strategyId
      if (patch.schedule !== undefined) {
        job.schedule = normalizeSchedule(patch.schedule)
    ...
    async remove(id) {
      ...
      if (index === -1) throw new Error(`trader job not found: ${id}`)
      jobs.splice(index, 1)
      await save()
    },
