  ...
  build(ctx: WorkspaceToolContext) {
    return tool({
      description: [
        ...
        '`effort`, `timeout`, `what`, `watch`, `watchPaused`, `expectedWatchVersion`, or `commentPrompt`; omitted fields are',
        ...
        'default wrapper). `watch` is the deterministic pre-dispatch monitoring gate (v1 whitelist in',
        'watch/spec.ts: one source + one interval, closed-bar price/EMA/VWAP/structure only; it requires `when`; null removes it;',
        '`expectedWatchVersion` guards stale-plan overwrites. Use `freshness.maxStaleMinutes` for intraday sources,',
        '`since` for structure events, and a narrow `lookbackBars` for zone touches. Richer conditions (multi-timeframe,',
        'order-flow, news) stay in `what` as post-hit analysis. `watchPaused` pauses watched dispatch',
        'with the plan kept (null/false resumes).',
      ...
      inputSchema: z.object({
        ...
        watch: z.unknown().optional().describe('Deterministic pre-dispatch gate — JSON watch plan (v1 whitelist: one source + one interval, closed-bar price/EMA/VWAP/structure); requires when; null removes it. Omitted fields are left untouched.'),
        expectedWatchVersion: z.number().int().min(1).optional().describe('Refuse the watch update when the live watch version differs (stale-plan protection; re-read the live issue first).'),
        watchPaused: z.boolean().nullable().optional().describe('Pause/resume watched dispatch with the plan and latch kept (independent switch); null/false clears to live.'),
  ...
  build(ctx: WorkspaceToolContext) {
    return tool({
      ...
      inputSchema: z.object({
        ...
        watch: z.unknown().optional().describe('Deterministic pre-dispatch monitoring gate — JSON watch plan (v1 whitelist in watch/spec.ts: one source + one interval, closed-bar price/EMA/VWAP/structure). Requires `when`; set intraday freshness and structure/zone lookbacks explicitly. Omit for no monitoring; richer logic stays in `what`.'),
