  ...
  build(ctx: WorkspaceToolContext) {
    return tool({
      description: [
        ...
        'watch/spec.ts: one source + one interval, closed-bar price/EMA/VWAP/structure only; null removes it;',
        '`expectedWatchVersion` guards stale-plan overwrites). Richer conditions (multi-timeframe,',
      ...
      inputSchema: z.object({
        ...
        watch: z.unknown().optional().describe('Deterministic pre-dispatch gate — JSON watch plan (v1 whitelist: one source + one interval, closed-bar price/EMA/VWAP/structure); null removes it. Omitted fields are left untouched.'),
