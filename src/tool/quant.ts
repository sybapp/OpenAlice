...
Federates connected brokers, user-enabled keyless data sources (binance-readonly, …), AND enabled vendors
...
Candidates come back relevance-first, then freshest within the same match quality. Each carries:
  - barId: use directly, e.g. bars("<barId>", "1d", count=250).
    · broker and native vendor barIds (for example tradingview|NASDAQ:AAPL) need NO asset= in bars().
    · compatibility vendor barIds need asset="equity|crypto|currency|commodity".
