...
describe('createTechnicalAnalysisTools', () => {
  it('exposes one unified technical-analysis entrypoint', () => {
    ...
    // numBins allocates one object per bin and is sorted twice downstream — bounded.
    expect(schema.safeParse({ barId: 'tradingview|AAPL', interval: '1h', numBins: 500 }).success).toBe(true)
    expect(schema.safeParse({ barId: 'tradingview|AAPL', interval: '1h', numBins: 50_000_000 }).success).toBe(false)
  ...
  it('keeps the interval analysis when the intrabar fetch fails', async () => {
    const target = targetBars()
    const barService: BarService = {
      searchBarSources: vi.fn(),
      getBars: vi.fn()
        ...
        .mockRejectedValueOnce(new Error('TradingView socket timeout')),
    }
    const tools = createTechnicalAnalysisTools({ barService })
    ...
    // Order flow degrades; Price Action, indicators and meta survive.
    expect(result.status).toBe('ok')
    expect(result.intervals[0].status).toBe('ok')
    expect(result.intervals[0].priceAction).toBeDefined()
    expect(result.intervals[0].indicators).toBeDefined()
    expect(result.intervals[0].meta).toMatchObject({ barId: 'tradingview|AAPL' })
    expect(result.intervals[0].orderFlow.status).toBe('no_intrabars')
    // The cause stays visible instead of being swallowed.
    expect(result.intervals[0].summary.warnings.join(' ')).toContain('TradingView socket timeout')
  })
})
