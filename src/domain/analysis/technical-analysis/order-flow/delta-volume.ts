...
/** 默认价格区间数量。 */
export const DEFAULT_PROFILE_BINS = 20
/**
 * Bin 数量上界。每个 bin 都是一个对象，buildProfileStructure 还会对整个数组按
 * 分位数排序两次，因此这个值必须有界 —— 它来自 agent 输入。
 */
export const MAX_PROFILE_BINS = 500

export interface VolumeProfileParams {
  bars: OhlcvBar[]
  /** 价格区间数量（默认 20，上限 MAX_PROFILE_BINS） */
  numBins?: number
}
...
export function calculateVolumeProfile(params: VolumeProfileParams): VolumeProfileResult {
  const { bars } = params
  // Clamp at the allocator too, not only at the tool boundary: this function is
  // reachable from every order-flow caller and the bin count is agent-supplied.
  const requested = params.numBins ?? DEFAULT_PROFILE_BINS
  const numBins = Number.isFinite(requested)
    ? Math.min(MAX_PROFILE_BINS, Math.trunc(requested))
