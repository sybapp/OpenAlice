...
function candidatesFor(targetInterval: string, supportedIntervals?: readonly string[]): string[] {
  ...
  return candidates.filter((interval) => supported.has(interval) && (targetMinutes == null || intervalToMinutes(interval)! < targetMinutes))
}
...
export function chooseIntrabarPlan(
  ...
  if (candidates.length === 0) {
    return {
      intrabarInterval: 'unavailable',
      intrabarsPerParent: 0,
      requestedCount,
      actualCount: requestedCount,
      maxSupportedCount: 0,
      requiredIntrabarBars: 0,
      maxBars: MAX_INTRABAR_BARS,
      truncated: false,
      degradationReason: `No lower supported interval is available below ${targetInterval}.`,
    }
  }
  ...
  const intrabarInterval = candidates[candidates.length - 1]!
  const perParent = intrabarsPerParent(targetInterval, intrabarInterval)
