...
function summarizeIntervals(intervals: TechnicalAnalysisIntervalResult[]): TechnicalAnalysisSummary {
  ...
  // 0 vs 0 is "no confirmed structure anywhere", not "the timeframes disagree".
  // Reporting 'mixed' there tells the agent about a conflict that does not exist.
  ...
  const alignment: TechnicalAnalysisSummary['alignment'] = conflicts.length > 0
    ? 'conflicted'
    // Same reasoning as `bias` above: with no confirmed trend anywhere there is
    // nothing to align or mix, so the honest answer is 'unknown'.
    : distinctTrends.size === 0
      ? 'unknown'
