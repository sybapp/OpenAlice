...
export function createBarService(deps: BarServiceDeps): BarService {
  ...
  async function getNativeVendorBars(
    ...
    return {
      ...
      meta: buildMeta(symbol, bars, {
        ...
        ...computeFreshness(bars[bars.length - 1]?.date ?? '', opts, () => new Date(), bars[0]?.date ?? '', adapter.metadata.capability),
      }),
    }
  }
  ...
  async function getVendorBars(
    ...
    // Legacy alias kept for the commodity branch below until it migrates.
    const p = buildHistoricalParams
    ...
    return {
      ...
      meta: buildMeta(symbol, filtered, {
        ...
        ...computeFreshness(filtered[filtered.length - 1]?.date ?? '', opts, () => new Date(), filtered[0]?.date ?? '', getVendorCapability(provider)),
      }),
    }
  }
