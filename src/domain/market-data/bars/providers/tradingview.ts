...
const TRADINGVIEW_CAPABILITY = 'realtime' as const
...
export const TRADINGVIEW_VENDOR_INFO = {
  ...
  name: 'TradingView',
  keyless: true,
  coverage: 'Real-time global OHLCV for equities, crypto, and FX technical analysis; volume coverage depends on the exchange.',
  howToUse:
    ...
    'Use this source for technical analysis only; it is not an executable quote and is never used for order submission. ' +
    'Bare US equities may use partial-market Cboe volume rather than SIP consolidated volume.',
...
/** Asset classes this adapter can search, in a stable order. */
const SEARCHABLE_ASSET_CLASSES = ['equity', 'crypto', 'currency'] as const
export type TradingViewAssetClass = (typeof SEARCHABLE_ASSET_CLASSES)[number]

/**
 * Which asset classes TradingView search should cover for the current config.
 *
 * `extraVendors` is what the Settings toggle and `setMarketVendor` actually
 * write, and it carries no asset class — enabling TradingView there must light
 * up every class the adapter can search, not equities alone. Otherwise the
 * agent can never discover the `BINANCE:BTCUSDT` / `FX:EURUSD` addressing that
 * this vendor's own `howToUse` tells it to use and that `getBars` already
 * serves. A per-asset `providers` entry keeps working as an explicit opt-in.
 */
export function tradingViewAssetClasses(config: {
  providers: Partial<Record<string, string>>
  extraVendors: readonly string[]
}): TradingViewAssetClass[] {
  if (config.extraVendors.includes(TRADINGVIEW_PROVIDER_ID)) return [...SEARCHABLE_ASSET_CLASSES]
  return SEARCHABLE_ASSET_CLASSES.filter(
    (assetClass) => config.providers[assetClass] === TRADINGVIEW_PROVIDER_ID,
  )
}
...
function mapCandidate(row: TradingViewSearchHit, assetClass: AssetClass): BarSourceCandidate | null {
  ...
  return {
    ...
    name,
    assetClass,
    label: `${base} · ${TRADINGVIEW_CAPABILITY}`,
    barCapability: TRADINGVIEW_CAPABILITY,
...
export function createTradingViewBarAdapter(
  ...
  return {
    ...
    metadata: {
      capability: TRADINGVIEW_CAPABILITY,
      supportedIntervals: TRADINGVIEW_SUPPORTED_INTERVALS,
      supportsCount: true,
    },
