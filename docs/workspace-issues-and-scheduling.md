...
- `watch` — optional deterministic pre-dispatch monitoring gate for a scheduled monitoring Issue; it requires `when`. Shape: `{ version, source: { barId, interval, assetClass? }, sources?, quote?, freshness?, indicators?, rule }` with one leaf or one `{ all: [...] }` / `{ any: [...] }` group (1–8 leaves, no nesting). Leaf whitelist: closed-bar `price_above` / `price_below` / `price_in_range` / `price_out_of_range` / `price_cross_above` / `price_cross_below` / `price_touch`, `volume_spike` / `cvd_slope` / `price_volume_divergence` (bar-proxy volume on loaded bars, `fidelity: 'bar_proxy'`), `ema_alignment` / `price_vs_ema` / `price_vs_vwap`, `structure_break` (BOS/CHoCH), `zone_touch` (FVG/OB). `price_touch` overlaps a price level with the high/low range of recent closed bars; an intraday touch requires `freshness.maxStaleMinutes`. `source` is the default context; `sources` adds at most four named lowercase contexts (`^[a-z][a-z0-9-]{0,31}$`) and a leaf may select one with `source: '<name>'`. Each context is fetched, freshness-gated, and computed independently, so mixed-interval `all` / `any` rules are supported. A check is bounded to five contexts, 1,000 total bars (`WATCH_MAX_TOTAL_BARS`), and four concurrent fetches. Intrabar order-flow (absorption, exhaustion, footprint) and realtime/forming-bar touch conditions remain post-hit analysis. Watch checks use the Issue cadence; a miss/unavailable result advances the check cursor without advancing the dispatch marker. A hit dispatches once per arming **per leaf**: in a mixed rule each leaf latches independently (a signal-less price/indicator leaf re-fires on a band re-entry within the same plan; a signal-bearing structure/zone leaf dedups on its own ids only, so a stale structure signal never covers a fresh price hit and vice versa). For intraday watches set `freshness.maxStaleMinutes`; for `structure_break` set `since` when historical events must not count, and keep `zone_touch.lookbackBars` narrow. The run opens with a `<watch-verdict>` block that marks the triggering leaf(ves) `[fresh]` and exits by re-arming (`expectedWatchVersion` guards stale plans), proposing a trade, or closing. Unknown types are invalid files, never silent misses.
- `watchPaused` — optional pause for watched dispatch only and requires `watch`. The plan and latch are kept; resume continues the same arming. Omission is live.
