...
1. **Latch after hit — one latch per leaf.** Each leaf in a mixed rule latches independently: a signal-less (price/indicator) leaf stays silent while its hit holds, re-arms its latch on a miss (band exit; re-entry re-fires without a version bump), and keeps its latch on `unavailable` (a data gap is not a band exit); a signal-bearing (structure/zone) leaf keeps its consumed ids on every non-dispatch tick and deduplicates on its own ids only, so a stale structure signal never covers a fresh price leaf (and vice versa). A still-true condition does not re-fire. Re-arm requires the harness to write a new `watch` (bumped `version`) or an explicit re-arm. Cooldown is auxiliary only.
...
- [x] `watch-state.json` (`schedule/watch-state.ts`): `{ watchVersion, lastCheckedAt, lastTriggeredAt, lastStatus, lastReason, lastEvidence, lastRunId, leafStates }`, atomic write, per-scan prune, restart reload dedups. The branch-only flat `consumedSignalIds` shape was replaced directly (never shipped, no migration path).
- [x] Latch — per-leaf (scanner `fireWatched` + `buildLeafStates` + `isLeafFresh`): each leaf latches on its own state; signal-bearing leaves keep their consumed ids on every non-dispatch tick (an idle sibling never loses history) and fold current ids in only on dispatch; signal-less leaves latch on per-leaf `lastTriggeredAt`, cleared on miss (band re-entry re-fires within the same arming) but preserved on `unavailable` (a data gap is not a band exit). The verdict block marks the fresh leaf `[leaf[i]: hit [fresh]]`. Capacity/busy skip consumes nothing (fresh hits stay live for the next tick). Check throw isolates per-issue as `unavailable`.
